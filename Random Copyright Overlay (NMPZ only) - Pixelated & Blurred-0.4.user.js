...
(function() {
  ...
  const MAX_FS      = 20;
  const MAX_SHEAR   = 55;
  const PX_FACTOR   = 1.5;  // draw at 25% resolution
  ...
  function makeOverlay(svCanvas) {
    ...
    function buildLegendImage(year, fs) {
      ...
      // draw the string scaled down
      ctx.scale(PX_FACTOR, PX_FACTOR);
      ctx.font = `${year>=2021?'bold ':''}${fs}px "Arial Narrow",sans-serif`;
